...
export default function CalendarView({ sessionAt, slotOverrides, toggleSlot, slotKey, weekDates, calendarDate, setCalendarDate }) {
  ...
  const handleShiftWeek = (n) => setCalendarDate(toISO(addDays(parseISO(calendarDate), n * 7)));
  const rangeLabel = `${fmtDay(weekDates[0])} – ${fmtDay(weekDates[6])}, ${weekDates[0].getFullYear()}`;

  return (
    <div className="fceo-section">
      <div className="fceo-section-head">
        <div><h2>Weekly Calendar</h2><p className="fceo-muted">Auto-filled from each student's fixed days/time. Click any open cell to mark it Available or Blocked.</p></div>
      ...
      <div className="fceo-cal-nav">
        <button className="fceo-btn" onClick={() => handleShiftWeek(-1)}><Icon name="chevL" /> Prev week</button>
        <button className="fceo-btn" onClick={goToday}>Today</button>
        <button className="fceo-btn" onClick={() => handleShiftWeek(1)}>Next week <Icon name="chevR" /></button>
            ...
            {HOURS.map((h) => (
              <tr key={h}>
                ...
                {weekDates.map((d) => {
                  ...
                  const sess = sessionAt(d, h);
                  const override = slotOverrides[slotKey(dayAbbr, h)] || "open";
                  if (sess) return <td key={toISO(d)} className="fceo-cal-cell session" title={sess.fullName}>{sess.fullName.split(" ")[0]}</td>;
                  return <td key={toISO(d)} className={"fceo-cal-cell " + override} onClick={() => toggleSlot(dayAbbr, h)} />;
                })}
              </tr>
            ))}
          </tbody>
