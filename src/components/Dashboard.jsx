...
export default function Dashboard({ todaysSessions, stats, expectedMonthlyIncome, collectedThisMonth, activeStudents, leads }) {
  ...
  return (
    <div className="fceo-grid">
      ...
      <section className="fceo-card">
        ...
        <Stat label="Win rate" value={stats.winRate === null ? "—" : stats.winRate.toFixed(1) + "%"} accent={stats.winRate >= 50 ? "up" : "down"} />
        <Stat label="Total P&L" value={fmtZAR(stats.totalPnl)} accent={stats.totalPnl >= 0 ? "up" : "down"} />
        <Stat label="Avg R:R outcome" value={(stats.avgR >= 0 ? "+" : "") + stats.avgR.toFixed(2) + "R"} />
        <Stat label="Trades logged" value={stats.total} />
      </section>
    </div>
  );
}